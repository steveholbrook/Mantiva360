...
Baseline source commit: `d68ee959abfa8fdccd75701fcc4cb3de152c93c2`.
...
| Product proof | Strong and clearly labelled | GOLD2 screenshots are identified as actual product captures using fictional data; explanatory graphics are labelled; full screenshots remain available. | A current genuine Planning & Forecasts capture was not available, so that panel remains an explanatory graphic. |
...
- The principal HTML, CSS, JavaScript, configuration and five image assets totalled about 356 KB before transfer compression. No framework or initial video payload is present.
